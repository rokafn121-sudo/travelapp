...
# PWA / 홈 화면 아이콘 설정 (근본적인 Streamlit index.html 패치 및 JS fallback)
def patch_streamlit_index():
    ...
    try:
        ...
        # 파일 기반으로 서빙되도록 Streamlit 패키지 정적 폴더에 복사
        target_icon_path = os.path.join(static_dir, "apple-touch-icon.png")
        target_favicon_path = os.path.join(static_dir, "favicon.png")
        target_manifest_path = os.path.join(static_dir, "manifest.json")
        
        if os.path.exists(profile_path):
            shutil.copy(profile_path, target_icon_path)
            shutil.copy(profile_path, target_favicon_path)
        ...
        # 기존 태그 걷어내기
        html = re.sub(r'<!-- CUSTOM PWA TAGS START -->.*?<!-- CUSTOM PWA TAGS END -->\n?', '', html, flags=re.DOTALL)
        ...
        new_html = html.replace("</head>", pwa_tags)
